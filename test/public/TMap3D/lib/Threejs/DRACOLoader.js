...
THREE.DRACOLoader.prototype = Object.assign( Object.create( THREE.Loader.prototype ), {
	...
	decodeGeometry: function ( buffer, taskConfig ) {
		...
		var geometryPending = this._getWorker( taskID, taskCost )
			...
			.then( function( message ){return that._createGeometry( message.geometry )} );
		...
		geometryPending
			.finally( function(){
				...
				if ( worker && taskID ) {
					...
					that._releaseTask( worker, taskID );
	...
	_getWorker: function ( taskID, taskCost ) {
		...
		return this._initDecoder().then( function(){
			...
			if ( that.workerPool.length < that.workerLimit ) {
				...
				worker.postMessage( { type: 'init', decoderConfig: that.decoderConfig } );
	...
	debug: function () {
		...
		console.log( 'Task load: ', this.workerPool.map( function( worker ) {return worker._taskLoad; }) );
